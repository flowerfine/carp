 ...
 */
package cn.sliew.carp.plugin.workflow.engine.internal.core.util;
...
public enum DagExecutionUtil {
    ...
    public static boolean anyUpstreamStepsFailed(DAG<WorkflowStepInstance> dag, WorkflowStepInstance stepInstance) {
        return dag.inDegreeOf(stepInstance).stream()
                .anyMatch(s -> {
                    CarpWorkflowStepInstanceState stepInstanceState = CarpWorkflowStepInstanceState.of(s.getStatus());
                    if (stepInstanceState.isFailureOrShutdown()) {
                        return true;
                    }
                    return anyUpstreamStepsFailed(dag, s);
                });
    }

    public static Optional<WorkflowStepInstance> findAnyUpstreamFailedStep(DAG<WorkflowStepInstance> dag, WorkflowStepInstance stepInstance) {
        return dag.getAncestors(stepInstance).stream()
                .filter(s -> {
                    CarpWorkflowStepInstanceState stepInstanceState = CarpWorkflowStepInstanceState.of(s.getStatus());
                    return stepInstanceState.isFailureOrShutdown();
                }).findFirst();
    }
