 ...
 */
package cn.sliew.carp.plugin.workflow.engine.internal.spinnaker.queue.handler;
...
import cn.sliew.carp.plugin.workflow.engine.internal.domain.instance.TaskExecution;
import cn.sliew.carp.plugin.workflow.engine.internal.domain.instance.TaskExecutionImpl;
import cn.sliew.carp.plugin.workflow.engine.internal.domain.instance.WorkflowStepInstance;
import cn.sliew.carp.plugin.workflow.engine.internal.spinnaker.exceptions.StepTimeoutException;
import cn.sliew.carp.plugin.workflow.engine.internal.spinnaker.queue.Messages;
import cn.sliew.carp.plugin.workflow.engine.internal.spinnaker.util.DagExecutionUtil;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.resolver.NoSuchTaskException;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.resolver.TaskResolver;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.task.RetryableTask;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.task.Task;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.task.TaskExecutionInterceptor;
import cn.sliew.carp.plugin.workflow.engine.internal.stage.model.task.TaskResult;
...
@Component
public class RunTaskHandler2 extends AbstractWorkflowMessageHandler<Messages.RunTask> {
    ...
    private void withLocking(Messages.RunTask message, Runnable action) {
        LockRunResult<Void> lockRunResult = lockAndRunExecutor.execute(action, String.format("carp:workflow:%s:%s:%d:%d",
                message.getNamespace(), message.getType(), message.getWorkflowInstanceId(), message.getStepId()));
        if (!lockRunResult.isLockAcquired()) {
            getLog().warn("Workflow Step (namespace: {}, type: {}, workflowInstanceId: {}, stepId: {}) fail to obtain lock for task. Pushing original message back to queue",
                    message.getNamespace(), message.getType(), message.getWorkflowInstanceId(), message.getStepId());
...
//            registry.counter(getTimeoutCounterId(stage, taskModel)).increment();
            ...
            if (set.contains(timeoutResult.getStatus())) {
                getLog().error("Workflow Task (namespace: {}, type: {}, workflowInstanceId: {}, stepId: {}, task: {}) return invalid status ({}) for onTimeout",
                        message.getNamespace(), message.getType(), message.getWorkflowInstanceId(), message.getStepId(),
...
//        trackResult(stage, startTimeMs, taskModel, result.getStatus());
    ...
    private void handleTaskException(Messages.RunTask message,
        ...
        if (exceptionVO != null && exceptionVO.isRetryable()) {
            getLog().warn("Workflow Task (namespace: {}, type: {}, workflowInstanceId: {}, stepId: {}, task: {}) run error, retry",
                    message.getNamespace(), message.getType(), message.getWorkflowInstanceId(), message.getStepId(), message.getTaskType().getSimpleName());
            push(message, getBackoffPeriod(task, taskModel, stepInstance));
...
//                            e);
//                } else {
                getLog().error("Workflow Task (namespace: {}, type: {}, workflowInstanceId: {}, stepId: {}, task: {}) run error, terminal",
                        message.getNamespace(), message.getType(), message.getWorkflowInstanceId(), message.getStepId(), message.getTaskType().getSimpleName());
            }
