...
import cn.sliew.carp.module.workflow.stage.model.task.WaitTask;
    ...
    public void taskGraph(@Nonnull StageExecution stage, @Nonnull TaskNode.Builder builder) {
        ...
        try {
            ...
            if (jitter.enabled && jitter.maxDelay > 0) {
                if (jitter.skipManual && stage.getPipelineExecution().getTrigger().getType().equals("manual")) {
                    return;
                }

                long waitTime = ThreadLocalRandom.current().nextLong(jitter.minDelay, jitter.maxDelay + 1);

                ((StageExecutionImpl) stage).setContext(contextWithWait(stage.getContext(), waitTime));
    ...
    private static class SuspendExecutionDuringTimeWindowTask implements RetryableTask {
        ...
        public @Nonnull TaskResult execute(@Nonnull StageExecution stage) {
            ...
            if (now.equals(scheduledTime) || now.isAfter(scheduledTime)) {
                ...
                ((StageExecutionImpl) stage).setScheduledTime(scheduledTime);
