...
public class CarpLogDataSourceConfig {

    @Autowired
    private MybatisPlusInterceptor mybatisPlusInterceptor;

    @Bean(DataSourceConstants.LOG_DATA_SOURCE_FACTORY)
    @ConfigurationProperties(prefix = "spring.datasource.carp.log")
    public DataSource carpLogDataSource() {
        return DataSourceBuilder.create().type(HikariDataSource.class)
                .build();
    }

    @Bean(DataSourceConstants.LOG_TRANSACTION_MANAGER_FACTORY)
    public DataSourceTransactionManager carpLogTransactionManager() {
        return new DataSourceTransactionManager(carpLogDataSource());
    }

    @Bean(DataSourceConstants.LOG_SQL_SESSION_FACTORY)
    public SqlSessionFactory carpLogSqlSessionFactory() throws Exception {
        ...
        factoryBean.setDataSource(carpLogDataSource());
